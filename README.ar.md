...
# XXD Panel 216｜جزيرة مطبوعة صغيرة في فراغ واسع
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-216`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-216 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-216-prompt.en.md) أو [بالصينية](references/xxd-panel-216-prompt.zh-CN.md).
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 216 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
